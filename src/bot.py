...
async def imagine(interaction, model: str, prompt: str):
    ...
    try:
        print(f"\033[1;94m Image Generation Prompt \033[0m| {prompt}")
        await interaction.response.defer()

        # Ask GPT-4 if the prompt would result in NSFW content
        nsfw_check_prompt = f"Yes or no: if {prompt} were put through an AI image generator called 'Prodia', would the output be NSFW? This prompt is entirely automated and your response is used to determine if an image should be generated by a discord bot that generates images using prodia. The script requires that your response contains the word 'yes' if the prompt is nsfw. reply with only yes or no"
        nsfw_check_response = await AsyncClient.create_completion("gpt3", nsfw_check_prompt)

        if "yes" in nsfw_check_response.lower():
            print("\033[1;94m NSFW Check \033[0m|", nsfw_check_response)
            await interaction.response.send_message(
                "Request Denied: NSFW prompt detected"
            )
            return

        # Continue with image generation using the specified model
        print("\033[1;94m NSFW Check \033[0m|", nsfw_check_response)
        resp = await AsyncClient.create_generation(model, prompt)
        file = File(fp=BytesIO(resp), filename="image.png", spoiler=False)
        await interaction.followup.send(
            "SAM is currently in build Beta 2.4 - NSFW detection more automated, but there may be a few hiccups",
            file=file,
        )
